...
class PartsPerBoxReport(Model):
    def __init__(self) -> None:
        super().__init__()
    
    _name = 'parts_per_box_report'
    _id_column = 'id'
    _init = True
    _get_name_string = '{id}'
    _order = '[order], pallet, box, item_set, side'
    ...
    def load_stored_function(self, context=None):
        query = f"""
            parts_per_box_report({context.get('box') or 0})
        """
        return query

    def init(self):
        self.drop_function_if_exists('parts_per_box_report')
        query = """
            CREATE FUNCTION [parts_per_box_report] (@box_id int)
            RETURNS 
            TABLE 
            AS	
            RETURN 
    ...
    def translate(self, params={}):
        ...
        return translation or super(PartsPerBoxReport, self).translate(params=params)
